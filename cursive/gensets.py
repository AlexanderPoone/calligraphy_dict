...
CLASS_NAMES = np.array([bytes(item.name[0], encoding='utf8') for item in pathlib.Path("gen_test").glob('*')])
print(CLASS_NAMES)
def decode_img(img):
...
q_list_ds = tf.data.Dataset.list_files('gen_test/*')
q_labeled_ds = q_list_ds.map(process_path) #, num_parallel_calls=AUTOTUNE)
q_train_ds = prepare_for_training(q_labeled_ds)
...
q_image_batch, q_label_batch = next(iter(q_train_ds))
show_batch(image_batch.numpy(), label_batch.numpy())

history = model.fit(image_batch, label_batch, epochs=30, 
                    validation_data=(t_image_batch, t_label_batch))

image_batch, label_batch = next(iter(q_train_ds))
print([str(CLASS_NAMES[x], encoding='utf8') for x in model.predict_classes(image_batch)])
predicted = []
for lst in label_batch:
    for i,v in enumerate(lst):
        if v:
            predicted.append(i)
print([str(CLASS_NAMES[x], encoding='utf8') for x in predicted])
print(str(CLASS_NAMES[3], encoding='utf8'))
print(str(CLASS_NAMES[26], encoding='utf8'))





# preferred style function